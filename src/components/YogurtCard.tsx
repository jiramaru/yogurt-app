...
export default function YogurtCard({ yogurt }: YogurtCardProps) {
  ...
  return (
    <Card className="flex flex-col h-full transition-transform hover:scale-[1.01] max-w-xs mx-auto">
      <CardHeader className="p-3 pb-0">
        <CardTitle className="text-base font-semibold truncate">{yogurt.name}</CardTitle>
      </CardHeader>
      <CardContent className="flex-grow space-y-3 p-3">
        ...
        <div className="flex justify-between items-center">
          ...
          <p className={`text-xs ${yogurt.stock > 10 ? 'text-green-600 dark:text-green-400' : yogurt.stock > 0 ? 'text-amber-600 dark:text-amber-400' : 'text-red-600 dark:text-red-400'}`}>
            {yogurt.stock > 0 ? `In stock (${yogurt.stock})` : 'Out of stock'}
          </p>
        </div>
      </CardContent>
      <CardFooter className="p-3 pt-0">
        <Button 
          ...
          size="sm"
          aria-label={`Add ${yogurt.name} to cart`}
        >
          {yogurt.stock === 0 ? 'Out of Stock' : 'Add to Cart'}
        </Button>
      </CardFooter>
    </Card>
  );
